...
\paragraph{}A continuación veremos algunos métodos Model-Free.
...
\paragraph{}En nuestro caso, la estrategia utilizada para escoger la nueva acción es $\epsilon$-Greedy con $\epsilon$ valiendo 0.1, el cual funciona de la siguiente manera:
	

\paragraph{}próxima acción a tomar =  maxarg$_a\{Q(s,a)\}$ con probabilidad (1-e) y una acción al azar con probabilidad e.
...
\paragraph{}Sarsa es otro algoritmo libre de modelo la cual también utiliza la función de valores Q.

\paragraph{}El nombre, refleja el hecho que la función principal para actualizar los Q-valores depende en el estado actual, la acción que elije, el reward que obtiene, el estado en el que se encuentra luego de realizar la acción y finalmente la acción la cual el agente va a elegir en su próximo estado.

\paragraph{}Un agente Sarsa interectuará con el entorno y actualizará la política en base a acciones tomadas anteriormente. Los valores de la función Q son actualizados en base al error de predicción (ajustado por la tasa de aprendizaje ($\alpha$)).
