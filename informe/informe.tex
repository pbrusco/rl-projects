...
\end{figure} 
	...
	Los algoritmos que se ven en la figura \ref{fig:comparaNoRewardsFree} corrieron para un mapa de tamaño 5. 
	
	Lo primero que se puede observar es que ni sarsa ni sarsa($\lambda$) están convergiendo. Lo más probable es que esto esté sucediendo por los parámetros utilizados, ya que en otras implementaciones, pudimos observar que se utilizaban valores como 42412. Creemos que modificando estos parámetros nuestros algoritmos pueden llegar a converger más rápidamente, pero no dispusimos del tiempo necesario para hacer las pruebas.
	
	 En sarsa($\lambda$) lo que sucede es que se están fortaleciendo experiencias que no dieron buen resultado, es posible que bajando la cantidad de veces que se fortalecen las experiencias (N) o cambiando la importancia que se le da a esas experiencias, se logren mejores resultados.
	
	Sin embargo QLearning si converge a partir de juego 400.
...
StochasticExplo.Rmax-result.out & 8.86e-02 & 4 & 12 \\
StochasticNav.QLearn-result.out & 5.74e-05 & 372 & 13 \\
StochasticNav.Sarsa-result.out & 4.47e-05 & None & 13 \\
StochasticNav.Dyna-result.out & 3.38e-03 & 3041 & 14 \\
StochasticNav.Rmax-result.out & 4.93e-01 & 25 & 11 \\
\end{tabular}
...
\subsection{Rmax y rmax factorizado}

En ambos casos, los resultados son negativos, y nos dejan con sabor amargo.
